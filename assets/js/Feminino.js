// Feminino.js — versão final (idêntica à masculina)
// =====================================
...
// 3) Elementos DOM
// =====================================
const femininoList = document.getElementById("feminino-list");
const addBtn = document.getElementById("feminino-addProduct");

const modal = document.getElementById("feminino-modal");
const closeModal = document.querySelector(".feminino-close");
const modalTitle = document.getElementById("feminino-modalTitle");
const form = document.getElementById("feminino-form");

const modalName = document.getElementById("feminino-modalName");
const modalPrice = document.getElementById("feminino-modalPrice");
const modalImg = document.getElementById("feminino-modalImg");
const modalLink = document.getElementById("feminino-modalLink");
const modalCategoria = document.getElementById("feminino-modalCategoria");
const modalTamanho = document.getElementById("feminino-modalTamanho");
const modalDisponibilidade = document.getElementById("feminino-modalDisponibilidade");
...
// 4) Carregar produtos
// =====================================
async function loadFemininoProducts() {
  try {
    const { data, error } = await supabase.from("feminino").select("*").order("id", { ascending: true });
    if (error) throw error;
    allProducts = Array.isArray(data) ? data : [];
    renderFemininoProducts(allProducts);
  } catch (err) {
...
// =====================================
function renderFemininoProducts(products) {
  if (!femininoList) return;
  femininoList.innerHTML = "";
  ...
  products.forEach(p => {
    ...
    card.className = "feminino-girl-card-inter";
    card.innerHTML = `
      <div class="feminino-girl-card-img">
        <img src="${p.img_url ?? ''}" alt="${escapeHtml(p.name ?? '')}">
      </div>
      <div class="feminino-girl-card-info">
        <div class="feminino-girl-card-inform">
        ...
        <div class="feminino-girl-card-link">
          <a href="${p.link ?? '#'}" target="_blank">&leftrightsquigarrow;</a>
        </div>
      </div>
      ${isAdmin() ? `
      <div class="feminino-admin-actions">
        <button class="feminino-btn-edit" data-id="${p.id}">Editar</button>
        <button class="feminino-btn-delete" data-id="${p.id}">Excluir</button>
      </div>` : ""}
    `;
    femininoList.appendChild(card);
  });
  attachFemininoListeners();
}
...
// 7) Modal
// =====================================
function openFemininoModal(isEdit = false, data = null) {
  if (!modal) return;
...
if (form) {
  form.addEventListener("submit", async e => {
    ...
    try {
      if (editId) {
        await supabase.from("feminino").update(produto).eq("id", editId);
      } else {
        await supabase.from("feminino").insert([produto]);
      }
      modal.classList.add("hidden");
      await loadFemininoProducts();
    } catch (err) {
...
// 9) Editar / Excluir
// =====================================
function attachFemininoListeners() {
  document.querySelectorAll(".feminino-btn-edit").forEach(btn => {
    btn.onclick = async () => {
      const id = parseInt(btn.dataset.id);
      const { data } = await supabase.from("feminino").select("*").eq("id", id).maybeSingle();
      if (data) openFemininoModal(true, data);
    };
  });

  document.querySelectorAll(".feminino-btn-delete").forEach(btn => {
    btn.onclick = async () => {
      if (!confirm("Excluir produto?")) return;
      await supabase.from("feminino").delete().eq("id", parseInt(btn.dataset.id));
      loadFemininoProducts();
    };
  });
}
...
applyFilters?.addEventListener("click", async () => {
  if (!allProducts.length) await loadFemininoProducts();
  const filtrados = filtrarLocalmente();
  renderFemininoProducts(filtrados);
...
// 11) Inicialização
// =====================================
if (addBtn) addBtn.addEventListener("click", () => openFemininoModal(false));
loadFemininoProducts();
