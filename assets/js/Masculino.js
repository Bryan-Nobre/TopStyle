// Masculino.js — versão final (com filtro corrigido)
// =====================================
...
// 3) Elementos DOM
// =====================================
const masculinoList = document.getElementById("masculino-list");
const addBtn = document.getElementById("masculino-addProduct");

const modal = document.getElementById("masculino-modal");
const closeModal = document.querySelector(".masculino-close");
const modalTitle = document.getElementById("masculino-modalTitle");
const form = document.getElementById("masculino-form");

const modalName = document.getElementById("masculino-modalName");
const modalPrice = document.getElementById("masculino-modalPrice");
const modalImg = document.getElementById("masculino-modalImg");
const modalLink = document.getElementById("masculino-modalLink");
const modalCategoria = document.getElementById("masculino-modalCategoria");
const modalTamanho = document.getElementById("masculino-modalTamanho");
const modalDisponibilidade = document.getElementById("masculino-modalDisponibilidade");
...
// 4) Carregar produtos
// =====================================
async function loadMasculinoProducts() {
  try {
    const { data, error } = await supabase.from("masculino").select("*").order("id", { ascending: true });
    if (error) throw error;
    allProducts = Array.isArray(data) ? data : [];
    renderMasculinoProducts(allProducts);
  } catch (err) {
...
// =====================================
function renderMasculinoProducts(products) {
  if (!masculinoList) return;
  masculinoList.innerHTML = "";
  ...
  products.forEach(p => {
    ...
    card.className = "masculino-boy-card-inter";
    card.innerHTML = `
      <div class="masculino-boy-card-img">
        <img src="${p.img_url ?? ''}" alt="${escapeHtml(p.name ?? '')}">
      </div>
      <div class="masculino-boy-card-info">
        <div class="masculino-boy-card-inform">
        ...
        <div class="masculino-boy-card-link">
          <a href="${p.link ?? '#'}" target="_blank">&leftrightsquigarrow;</a>
        </div>
      </div>
      ${isAdmin() ? `
      <div class="masculino-admin-actions">
        <button class="masculino-btn-edit" data-id="${p.id}">Editar</button>
        <button class="masculino-btn-delete" data-id="${p.id}">Excluir</button>
      </div>` : ""}
    `;
    masculinoList.appendChild(card);
  });
  attachMasculinoListeners();
}
...
// 7) Modal
// =====================================
function openMasculinoModal(isEdit = false, data = null) {
  if (!modal) return;
...
if (form) {
  form.addEventListener("submit", async e => {
    ...
    try {
      if (editId) {
        await supabase.from("masculino").update(produto).eq("id", editId);
      } else {
        await supabase.from("masculino").insert([produto]);
      }
      modal.classList.add("hidden");
      await loadMasculinoProducts();
    } catch (err) {
...
// 9) Editar / Excluir
// =====================================
function attachMasculinoListeners() {
  document.querySelectorAll(".masculino-btn-edit").forEach(btn => {
    btn.onclick = async () => {
      const id = parseInt(btn.dataset.id);
      const { data } = await supabase.from("masculino").select("*").eq("id", id).maybeSingle();
      if (data) openMasculinoModal(true, data);
    };
  });

  document.querySelectorAll(".masculino-btn-delete").forEach(btn => {
    btn.onclick = async () => {
      if (!confirm("Excluir produto?")) return;
      await supabase.from("masculino").delete().eq("id", parseInt(btn.dataset.id));
      loadMasculinoProducts();
    };
  });
}
...
applyFilters?.addEventListener("click", async () => {
  if (!allProducts.length) await loadMasculinoProducts();
  const filtrados = filtrarLocalmente();
  renderMasculinoProducts(filtrados);
...
// 11) Inicialização
// =====================================
if (addBtn) addBtn.addEventListener("click", () => openMasculinoModal(false));
loadMasculinoProducts();
